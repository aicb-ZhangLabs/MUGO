...
# ==========================================
# 1. Multi-Head Selector (完全保留 V7 STE)
# ==========================================
...
def train(gene_name_arg, k_arg, tissue_arg, track_idx_arg, tau_arg):
    # 彻底锁死 seed 为 42
    set_seed(42)
    
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    LR, STEPS, INIT_TAU, MIN_TAU = 0.05, 200, tau_arg, 0.1
    ...
    # 2. 自动建立分层文件夹 (按要求对接 bash 脚本路径)
    BASE_DIR = '/home/dongbos/Combine_optim_Borzoi_SNP'
    DATASET_DIR = f'{BASE_DIR}/dataset'
    
    RESULT_DIR = f'{BASE_DIR}/rebuttal/results_rebuttal/sensitivity_analysis_results/sensitivity_analysis_tau/raw_csv_resutls'
    ...
    print(f"🚀 Optimizing {gene_name} for {tissue_arg} (Initial Tau: {tau_arg})...")
    history_log = []
    ...
    for step in range(STEPS):
        ...
        with torch.no_grad():
            ...
            row_data = {
                ...
                "Baseline": baseline_expr.item(), "Current_Tau": tau, "Initial_Tau": tau_arg,
        ...
        # 满足要求的每 10 个 epoch 打印
        if (step + 1) % 10 == 0:
            print(f"Step {step+1:3d} | Initial Tau: {tau_arg} | Current Tau: {tau:.4f} | Gain: {expr.item() - baseline_expr.item():+.2f}")

    # 7. 保存结果
    csv_filename = f"{RESULT_DIR}/{gene_name}_RNA_{tissue_arg}_tau{tau_arg}.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tau', type=float, required=True, help='Initial tau') # 5.0
