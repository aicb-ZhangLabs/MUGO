...
def prepare_tensors(csv_path, fasta_path, chrom, center_pos, seq_len=SEQ_LEN):
    ...
    snp_indices_list, snp_meta_list = [], []
    mapping = {'A': 0, 'C': 1, 'G': 2, 'T': 3}
...
def get_gene_meta_by_name(target_gene_name, meta_csv_path):
    ...
    row = row.iloc[0]
    return f"chr{row['chr']}", int(row['pos']), row['strand'], row['gene_ID'], row['gene_name']
...
# 3. Main Routine
# ==========================================
def train(gene_name_arg, k_arg, tissue_arg, seed_arg):
    # 锁定随机种子
    set_seed(seed_arg)
    print(f"🌱 Random Seed Locked: {seed_arg}")
    ...
    RESULT_DIR = f'{BASE_DIR}/rebuttal/results_rebuttal/random_seed_RNA_top100gene/random_seed_{seed_arg}/raw_results_csv_eachgene'
    ...
    ref_seq, alt_seq, snp_positions = ref_seq.to(DEVICE), alt_seq.to(DEVICE), snp_positions.to(DEVICE)
    
    # 🔥 修复 Bug：去掉了多余的 gene_name 参数
    exon_regions = get_exons_from_gtf(gene_id, GTF_PATH, tss, seq_start_pos)
    ...
    print(f"🚀 Optimizing {gene_name} for RNA {tissue_arg} (Seed {seed_arg})...")
    history_log = []
    ...
    csv_filename = f"{RESULT_DIR}/{gene_name}_RNA_{tissue_arg}_seed{seed_arg}.csv"
    pd.DataFrame(history_log).to_csv(csv_filename, index=False)
    print(f"✅ Finished {gene_name} (Seed {seed_arg}). Saved to {csv_filename}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--gene', type=str, required=True, help='Gene name (e.g., BBS2)')
    parser.add_argument('--seed', type=int, default=42, help='Random seed for reproducibility')
    parser.add_argument('--tissue', type=str, default='blood')
    parser.add_argument('--k', type=int, default=10)
    args = parser.parse_args()
    
    train(gene_name_arg=args.gene, k_arg=args.k, tissue_arg=args.tissue, seed_arg=args.seed)